...
export async function getLatestEpisodes(): Promise<Episode[]> {
  try {
    ...
    return response.episodes.length > 0 ? response.episodes : FALLBACK_EPISODES;
  } catch {
...
export async function getOnAirAnimes(): Promise<Anime[]> {
  try {
    ...
    return response.animes.length > 0 ? response.animes : FALLBACK_ANIMES;
  } catch {
...
export async function searchAnime(query: string): Promise<Anime[]> {
  try {
    ...
    if (response.animes.length > 0) return response.animes;
...
export async function getAnimesByGenre(genre: string, page = 1): Promise<Anime[]> {
  try {
    ...
    return response.animes.length > 0 ? response.animes : FALLBACK_ANIMES.filter(a => a.genres?.some(g => g.toLowerCase() === genre.toLowerCase()));
  } catch {
